...
class TakeAway extends React.Component {
    ...
    async componentDidMount (){ 
        console.log("Mounting the screen Take Away...");
        ...
        try {
            let response = await fetch('http://192.168.1.117/Ementas-de-Restauracao/index.php/TakeAway', { 
                headers: {
    ...
    goCreate() {
        this.props.history.push("/takeAwayCreate");
    }

    goEdit() {
        this.props.history.push("/takeAwayEdit");
    }
                    ...
                    <Container maxWidth="lg" className={"container"} style={{marginTop: 100}}>
                        <Grid container spacing={3}>
                            ...
                            <Paper elevation={3} className={"paper"}>
                                <label htmlFor="email">TakeAway</label>
                                <Table size="small">
                                    <TableHead>
                                    <TableRow>
                                    ...
                                    <TableBody>
                                    {
                                        data.map((item) => {
                                            return(
                                            <TableRow key={item.id_take_away}>
                                                <TableCell>{item.tipo_entrega}</TableCell>
                                                <TableCell align="right">{item.preco}</TableCell>
                                                <TableCell>{item.estado}</TableCell>
                                                <TableCell>{item.id_funcionario}</TableCell>
                                                <TableCell>{item.id_reserva}</TableCell>
                                            </TableRow>
                                            );
                                        })
                                    }
...
export default withRouter(withStyles(styles)(TakeAway));
