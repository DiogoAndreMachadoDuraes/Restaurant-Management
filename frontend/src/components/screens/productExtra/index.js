...
class ProductExtra extends React.Component {
    ...
    goCreate() {
        this.props.history.push("/productExtraExtraCreate");
    }

    goEdit() {
        this.props.history.push("/productExtraExtraEdit");
    }
                    ...
                    <Container maxWidth="lg" className={"container"} style={{marginTop: 100}}>
                        <Grid container spacing={3}>
                            ...
                            <Paper elevation={3} className={"paper"}>
                                <label htmlFor="email">Produtos Extra:</label>
                                <Table size="small">
                                    <TableHead>
                                    <TableRow>
                                        <TableCell>Quantidade</TableCell>
                                        <TableCell>Produto Correspondente</TableCell>
                                        <TableCell>Extra Correspondente</TableCell>
                                        <TableCell>
                                    ...
                                    <TableBody>
                                    {
                                        data.map((item) => {
                                            return(
                                            <TableRow key={item.id_produto_extra}>
                                                <TableCell>{item.quantidade}</TableCell>
                                                <TableCell>{item.id_produto}</TableCell>
                                                <TableCell align="right">{item.id_extra}</TableCell>
                                            </TableRow>
                                            );
                                        })
                                    }
